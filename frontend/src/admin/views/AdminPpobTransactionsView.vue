<template>
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <div class="rounded-lg border border-slate-200 bg-white dark:bg-[#1e2020] p-4 shadow-sm">
        <div class="flex items-center justify-between mb-1">
          <div class="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center">
            <HashIcon class="w-4 h-4 text-indigo-600" />
          </div>
          <span class="text-[10px] font-bold text-slate-400 dark:text-[#869392]">HARI INI</span>
        </div>
      ...
      <div class="rounded-lg border border-slate-200 bg-white dark:bg-[#1e2020] p-4 shadow-sm">
        <div class="flex items-center justify-between mb-1">
          ...
          <span class="text-[10px] font-bold text-slate-400 dark:text-[#869392]">VOLUME</span>
        </div>
      ...
      <div class="rounded-lg border border-slate-200 bg-white dark:bg-[#1e2020] p-4 shadow-sm">
        <div class="flex items-center justify-between mb-1">
          ...
          <span class="text-[10px] font-bold text-slate-400 dark:text-[#869392]">PROFIT</span>
        </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
      ...
      <select v-model="filterPeriod" class="h-9 px-3 text-sm border border-slate-200 dark:border-[#3d4948] rounded-lg outline-none" @change="onPeriodChange">
      ...
      <template v-if="filterPeriod === 'custom'">
        <input v-model="filterStartDate" type="date" class="h-9 px-3 text-sm border border-slate-200 dark:border-[#3d4948] rounded-lg outline-none" @change="fetchTransactions" />
        <input v-model="filterEndDate" type="date" class="h-9 px-3 text-sm border border-slate-200 dark:border-[#3d4948] rounded-lg outline-none" @change="fetchTransactions" />
      </template>
      <select v-model="filterStatus" class="h-9 px-3 text-sm border border-slate-200 dark:border-[#3d4948] rounded-lg outline-none" @change="fetchTransactions">
      ...
      <span v-if="meta" class="text-xs text-slate-500 dark:text-[#869392] self-center">{{ meta.total }} transaksi</span>
    </div>
    ...
    <div v-else-if="transactions.length === 0" class="bg-white dark:bg-[#1e2020] border border-dashed border-slate-200 rounded-lg p-10 text-center">
      ...
      <p class="text-sm font-semibold text-slate-700 dark:text-[#bcc9c7]">Belum ada transaksi PPOB</p>
    ...
    <div v-else class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full min-w-[800px]">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="trx in transactions" :key="trx.id" class="hover:bg-slate-50 dark:bg-[#1a1c1c]/30 cursor-pointer transition-colors" @click="openDetail(trx)">
              <td class="px-4 py-3 text-xs text-slate-600 dark:text-[#bcc9c7] whitespace-nowrap">{{ formatDateTime(trx.createdAt) }}</td>
              <td class="px-4 py-3 text-xs font-mono text-slate-700 dark:text-[#bcc9c7] whitespace-nowrap">{{ trx.refId.slice(-12) }}</td>
              <td class="px-4 py-3 text-xs font-semibold text-slate-800 dark:text-[#bcc9c7]">{{ trx.productCode }}</td>
              <td class="px-4 py-3">
                <p class="text-xs text-slate-800 dark:text-[#bcc9c7]">{{ trx.customerName || '-' }}</p>
                <p class="text-[10px] text-slate-500 dark:text-[#869392] font-mono">{{ trx.customerId }}</p>
              </td>
              ...
              <td class="px-4 py-3 text-xs text-right font-mono text-slate-800 dark:text-[#bcc9c7]">{{ formatRupiah(trx.amount) }}</td>
              ...
              <td class="px-4 py-3 text-[10px] font-mono text-slate-500 dark:text-[#869392] text-center max-w-[100px] truncate">{{ trx.serialNumber || '-' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <!-- Pagination -->
      <div class="px-4 py-3 border-t border-slate-200 dark:border-[#3d4948] flex items-center justify-between">
        <p class="text-xs text-slate-500 dark:text-[#869392]">Hal. {{ meta.page }} / {{ meta.totalPages }}</p>
        <div class="flex gap-1">
          <button type="button" :disabled="meta.page <= 1" class="px-3 py-1.5 text-xs font-semibold border border-slate-200 rounded-md disabled:opacity-50 hover:bg-slate-50 dark:bg-[#1a1c1c]" @click="goPage(meta.page - 1)">Prev</button>
          <button type="button" :disabled="meta.page >= meta.totalPages" class="px-3 py-1.5 text-xs font-semibold border border-slate-200 rounded-md disabled:opacity-50 hover:bg-slate-50 dark:bg-[#1a1c1c]" @click="goPage(meta.page + 1)">Next</button>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="detailTrx" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-[#1e2020] rounded-lg shadow-2xl w-full max-w-md p-6">
            <button type="button" class="absolute top-4 right-4 p-1" @click="detailTrx = null"><XIcon class="w-4 h-4 text-slate-400 dark:text-[#869392]" /></button>
            <h3 class="text-base font-bold text-slate-900 dark:text-[#e3e2e2] mb-4">Detail Transaksi PPOB</h3>
            <div class="space-y-2.5 text-sm">
              ...
              <div class="flex justify-between"><span class="text-slate-500 dark:text-[#869392] font-bold">Total</span><span class="font-mono font-bold">{{ formatRupiah(detailTrx.total) }}</span></div>
...
function statusBadge(s: string) {
  return { SUCCESS: 'bg-emerald-100 text-emerald-700', FAILED: 'bg-red-100 text-red-700', REFUNDED: 'bg-amber-100 text-amber-700', PROCESSING: 'bg-blue-100 text-blue-700' }[s] || 'bg-slate-100 text-slate-600 dark:text-[#bcc9c7]';
}
