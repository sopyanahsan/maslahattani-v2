<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
      <div class="relative flex-1 min-w-[200px]">
        ...
        <input v-model="searchCustomer" type="text" placeholder="Cari nama pelanggan..."
          class="w-full h-9 pl-9 pr-3 text-sm border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
    ...
    <div v-if="loading" class="flex items-center justify-center py-16">
      ...
      <span class="ml-2 text-sm text-slate-500 dark:text-slate-400">Memuat data hutang...</span>
    </div>
    ...
    <div v-else-if="debts.length === 0" class="bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-10 text-center">
      <HandCoinsIcon class="w-10 h-10 text-slate-300 dark:text-slate-600 mx-auto mb-3" />
      <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">Belum ada data hutang</p>
      <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Catat hutang pelanggan pertama lewat tombol di atas.</p>
    </div>
    ...
    <div v-else class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full min-w-[750px]">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            <tr v-for="debt in debts" :key="debt.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              <td class="px-4 py-3">
                <p class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ debt.customerName }}</p>
              ...
              <td class="px-4 py-3">
                <div class="w-full max-w-[120px] ml-auto">
                  <div class="flex items-center justify-between text-[10px] mb-1">
                    <span class="font-mono text-slate-600 dark:text-slate-400">{{ formatRupiah(debt.paidAmount) }}</span>
                    <span class="font-mono text-slate-900 dark:text-slate-100 font-bold">{{ formatRupiah(debt.totalAmount) }}</span>
                  </div>
                  ...
                  <p class="text-[10px] font-mono mt-0.5 text-right" :class="debt.totalAmount - debt.paidAmount > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'">
      ...
      <div v-if="meta && meta.totalPages > 1" class="px-4 py-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
        ...
        <div class="flex items-center gap-1">
          <button :disabled="meta.page <= 1" class="h-7 px-2.5 text-xs font-medium border border-slate-200 dark:border-slate-700 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 text-slate-700 dark:text-slate-300" @click="goPage(meta!.page - 1)">Prev</button>
          <button :disabled="meta.page >= meta.totalPages" class="h-7 px-2.5 text-xs font-medium border border-slate-200 dark:border-slate-700 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 text-slate-700 dark:text-slate-300" @click="goPage(meta!.page + 1)">Next</button>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showDetailModal && detailDebt" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md p-6 space-y-4 max-h-[85vh] overflow-y-auto">
          <div class="flex items-start justify-between">
            <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100">Detail Hutang</h3>
          ...
          <div>
            ...
            <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">{{ Math.round((detailDebt.paidAmount / detailDebt.totalAmount) * 100) }}% terbayar</p>
          </div>
          <!-- Payment history -->
          <div>
            <p class="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wide mb-2">Riwayat Cicilan ({{ detailDebt.debtPayments?.length || 0 }})</p>
            <div v-if="!detailDebt.debtPayments?.length" class="text-center py-4">
              <p class="text-xs text-slate-400 dark:text-slate-500">Belum ada pembayaran.</p>
            </div>
            <div v-else class="space-y-2">
              <div v-for="(p, idx) in detailDebt.debtPayments" :key="p.id" class="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 dark:border-slate-800">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-xs font-semibold text-slate-800 dark:text-slate-200">{{ formatRupiah(p.amount) }} <span class="text-[10px] font-normal text-slate-500 dark:text-slate-400">via {{ p.method }}</span></p>
                  <p class="text-[10px] text-slate-500 dark:text-slate-400">{{ formatDateTime(p.createdAt) }}<span v-if="p.notes"> · {{ p.notes }}</span></p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <form class="relative bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[85vh] overflow-y-auto" @submit.prevent="handleCreate">
          ...
          <div class="flex items-center justify-end gap-2 pt-2">
            <button type="button" class="h-9 px-4 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700" @click="showCreateModal = false">Batal</button>
    ...
    <Teleport to="body">
      <div v-if="showPayModal && payingDebt" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <form class="relative bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4" @submit.prevent="handlePay">
          ...
          <div><label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Metode <span class="text-red-500">*</span></label><select v-model="payForm.method" required class="w-full h-9 px-3 text-sm border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-md focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"><option value="CASH">Cash</option><option value="QRIS">QRIS</option><option value="TRANSFER">Transfer</option></select></div>
          <div><label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Catatan</label><input v-model="payForm.notes" type="text" placeholder="Cicilan ke-2" class="w-full h-9 px-3 text-sm border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-md focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></div>
          ...
          <div class="flex items-center justify-end gap-2 pt-2">
            <button type="button" class="h-9 px-4 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700" @click="showPayModal = false">Batal</button>
...
function debtStatusBadge(status: string): string {
  switch (status) {
    ...
    case 'CANCELLED': return 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400';
