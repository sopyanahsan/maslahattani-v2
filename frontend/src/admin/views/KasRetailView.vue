  ...
  <div class="space-y-5">
    ...
    <div class="flex gap-1 bg-slate-100 dark:bg-[#292a2a] rounded-lg p-1 w-fit">
      <button v-for="tab in tabs" :key="tab.key" type="button"
        :class="['h-8 px-4 text-xs font-semibold rounded-md transition-colors',
          activeTab === tab.key ? 'bg-white dark:bg-[#1e2020] text-slate-900 dark:text-[#e3e2e2] shadow-sm'
    ...
    <template v-if="activeTab === 'mutasi'">
      ...
      <div class="flex items-center justify-between">
        ...
        <button type="button"
          class="h-9 px-4 bg-[#03a29c] text-white text-xs font-semibold rounded-lg hover:bg-[#028a85] transition-colors flex items-center gap-1.5"
      ...
      <div class="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
        <!-- Total Pill -->
        <div class="shrink-0 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-[#292a2a] dark:to-[#1e2020] border border-slate-200/80 dark:border-[#3d4948] rounded-2xl px-5 py-3 shadow-sm">
          <p class="text-[9px] font-semibold uppercase tracking-wider text-slate-400 dark:text-[#869392] mb-0.5">Total Kas</p>
          <p class="text-lg font-bold font-mono tabular-nums text-slate-900 dark:text-[#e3e2e2] whitespace-nowrap leading-tight">{{ formatRupiah(cashBox?.balance ?? 0) }}</p>
        </div>
        <!-- Per-Kas Pill -->
        <div v-for="cb in cashBox?.cashBoxes || []" :key="cb.id"
          class="shrink-0 bg-white dark:bg-[#1e2020] border border-slate-200/80 dark:border-[#3d4948] rounded-2xl px-4 py-3 shadow-sm hover:shadow-md transition-shadow flex items-center gap-3 group">
          <div class="min-w-0">
            <div class="flex items-center gap-1.5 mb-0.5">
              <p class="text-[11px] font-semibold text-slate-700 dark:text-[#bcc9c7] truncate max-w-[120px]">{{ cb.label }}</p>
              <span v-if="cb.isDefault" class="w-1.5 h-1.5 rounded-full bg-[#03a29c] shrink-0" title="Default"></span>
            </div>
            <p class="text-sm font-bold font-mono tabular-nums whitespace-nowrap leading-tight" :class="cb.balance > 0 ? 'text-slate-900 dark:text-[#e3e2e2]' : 'text-red-500 dark:text-red-400'">{{ formatRupiah(cb.balance) }}</p>
          </div>
          <div class="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity shrink-0">
            ...
            <button type="button" class="w-6 h-6 rounded-full bg-slate-100 dark:bg-[#292a2a] text-slate-500 dark:text-[#bcc9c7] text-[10px] flex items-center justify-center hover:bg-slate-200 dark:hover:bg-[#3d4948] transition-colors" @click="filterByKas(cb)" title="Riwayat">↗</button>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        <select v-model="mutasiCategoryFilter" class="h-9 px-3 text-sm border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-lg focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" @change="fetchHistory">
          <option value="">Semua Kas</option>
          <option v-for="cb in cashBox?.cashBoxes || []" :key="cb.id" :value="cb.categoryId || cb.id">{{ cb.label }}</option>
        </select>
        <input v-model="mutasiStartDate" type="date" class="h-9 px-3 text-sm border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-lg focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" @change="fetchHistory" />
        <input v-model="mutasiEndDate" type="date" class="h-9 px-3 text-sm border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-lg focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" @change="fetchHistory" />
        <div class="flex-1"></div>
        <span v-if="historyMeta" class="text-xs text-slate-500 dark:text-[#869392] self-center">{{ historyMeta.total }} mutasi</span>
      </div>
      ...
      <!-- Empty -->
      <div v-else-if="historyData.length === 0" class="bg-white dark:bg-[#1e2020] border border-dashed border-slate-300 dark:border-[#3d4948] rounded-xl p-10 text-center">
        <WalletIcon class="w-12 h-12 text-slate-300 dark:text-[#3d4948] mx-auto mb-3" />
        <p class="text-sm font-bold text-slate-700 dark:text-[#bcc9c7]">Belum ada riwayat mutasi</p>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[600px]">
            <thead class="bg-slate-50 dark:bg-[#292a2a] border-b border-slate-200 dark:border-[#3d4948]">
              <tr>
                <th class="px-4 py-2.5 text-left text-[11px] font-bold text-slate-600 dark:text-[#bcc9c7] uppercase">Tanggal</th>
                <th class="px-4 py-2.5 text-center text-[11px] font-bold text-slate-600 dark:text-[#bcc9c7] uppercase">Tipe</th>
                <th class="px-4 py-2.5 text-right text-[11px] font-bold text-slate-600 dark:text-[#bcc9c7] uppercase">Jumlah</th>
                <th class="px-4 py-2.5 text-right text-[11px] font-bold text-slate-600 dark:text-[#bcc9c7] uppercase">Saldo Setelah</th>
                <th class="px-4 py-2.5 text-left text-[11px] font-bold text-slate-600 dark:text-[#bcc9c7] uppercase">Catatan</th>
              </tr>
            </thead>

            <tbody class="divide-y divide-slate-100 dark:divide-[#3d4948]">
              <tr v-for="item in historyData" :key="item.id" class="hover:bg-slate-50 dark:hover:bg-[#292a2a]">
                <td class="px-4 py-3 text-xs font-mono tabular-nums text-slate-600 dark:text-[#bcc9c7]">{{ formatDateTime(item.createdAt) }}</td>
                ...
                <td class="px-4 py-3 text-right text-xs font-mono tabular-nums font-semibold" :class="item.type === 'CASH_IN' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'">{{ item.type === 'CASH_IN' ? '+' : '-' }}{{ formatRupiah(item.amount) }}</td>
                <td class="px-4 py-3 text-right text-xs font-mono tabular-nums text-slate-700 dark:text-[#bcc9c7]">{{ formatRupiah(item.balanceAfter) }}</td>
                <td class="px-4 py-3 text-xs text-slate-600 dark:text-[#bcc9c7]">{{ item.notes || '—' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div v-if="historyMeta && historyMeta.totalPages > 1" class="px-4 py-3 border-t border-slate-200 dark:border-[#3d4948] flex items-center justify-between">
          <p class="text-xs text-slate-500 dark:text-[#bcc9c7]">Hal. {{ historyMeta.page }} / {{ historyMeta.totalPages }}</p>
          <div class="flex gap-1">
            <button :disabled="historyMeta.page <= 1" class="h-7 px-3 text-xs font-semibold border border-slate-200 dark:border-[#3d4948] rounded disabled:opacity-40 text-slate-700 dark:text-[#bcc9c7] hover:bg-slate-50 dark:hover:bg-[#292a2a]" @click="fetchHistoryPage(historyMeta!.page - 1)">Prev</button>
            <button :disabled="historyMeta.page >= historyMeta.totalPages" class="h-7 px-3 text-xs font-semibold border border-slate-200 dark:border-[#3d4948] rounded disabled:opacity-40 text-slate-700 dark:text-[#bcc9c7] hover:bg-slate-50 dark:hover:bg-[#292a2a]" @click="fetchHistoryPage(historyMeta!.page + 1)">Next</button>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-if="activeTab === 'kelola'">
      <!-- Actions bar -->
      <div class="flex items-center justify-between">
        <p class="text-sm text-slate-600 dark:text-[#bcc9c7]">Kelola kas retail. Setiap kas punya saldo sendiri.</p>
        <button type="button"
          class="h-9 px-4 text-xs font-semibold text-white bg-[#03a29c] rounded-lg hover:bg-[#028a85] transition-colors flex items-center gap-1.5"
          @click="openCashBoxModal(null)">
          <PlusIcon class="w-3.5 h-3.5" /> Tambah Kas
        </button>
      </div>
      ...
      <div v-if="kelolaLoading" class="flex items-center justify-center py-16">
        <Loader2Icon class="w-5 h-5 animate-spin text-[#03a29c]" />
        <span class="ml-2 text-sm text-slate-500 dark:text-[#bcc9c7]">Memuat kas...</span>
      </div>

      <!-- Empty -->
      <div v-else-if="kelolaKasList.length === 0" class="bg-white dark:bg-[#1e2020] border border-dashed border-slate-300 dark:border-[#3d4948] rounded-xl p-10 text-center">
        <BoxesIcon class="w-12 h-12 text-slate-300 dark:text-[#3d4948] mx-auto mb-3" />
        <p class="text-sm font-bold text-slate-700 dark:text-[#bcc9c7]">Belum ada kas</p>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <div v-for="kas in kelolaKasList" :key="kas.id"
          class="relative bg-white dark:bg-[#1e2020] border border-slate-200/80 dark:border-[#3d4948] rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group">
          <!-- Header -->
          <div class="flex items-center justify-between mb-2">
            <h4 class="text-sm font-bold text-slate-900 dark:text-[#e3e2e2] truncate pr-2">{{ kas.label }}</h4>
            <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <button class="w-6 h-6 rounded-full flex items-center justify-center hover:bg-slate-100 dark:hover:bg-[#292a2a] transition-colors"
                title="Edit" @click="openCashBoxModal(kas)">
                <PencilIcon class="w-3 h-3 text-slate-500 dark:text-[#bcc9c7]" />
              </button>
          ...
          <!-- Balance -->
          <p class="text-lg font-bold font-mono tabular-nums leading-tight mb-3" :class="kas.balance > 0 ? 'text-slate-900 dark:text-[#e3e2e2]' : 'text-red-500 dark:text-red-400'">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <button type="button"
              class="flex-1 h-7 text-[10px] font-semibold text-slate-600 dark:text-[#bcc9c7] bg-slate-50 dark:bg-[#292a2a] rounded-lg hover:bg-slate-100 dark:hover:bg-[#3d4948] transition-colors"
    ...
    <Teleport to="body">
      <div v-if="showMutationModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <form class="relative bg-white dark:bg-[#1e2020] rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4" @submit.prevent="handleMutation">
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] mb-1">Kas / Kategori *</label>
            <select v-model="mutationCategoryId" required class="w-full h-9 px-3 text-sm border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-md focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none">
              <option value="">— Pilih Kas —</option>
              <option v-for="cat in mutationCategories" :key="cat.id" :value="cat.id">{{ cat.name }} ({{ cat.code }})</option>
            </select>
          </div>
          <div><label class="block text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] mb-1">Jumlah (Rp) *</label><input v-model.number="mutationAmount" type="number" min="1" required class="w-full h-9 px-3 text-sm font-mono border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-md focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" /></div>
          <div><label class="block text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] mb-1">Catatan</label><input v-model="mutationNotes" type="text" placeholder="Opsional" class="w-full h-9 px-3 text-sm border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-md focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" /></div>
          ...
          <div class="flex justify-end gap-2 pt-2">
            <button type="button" class="h-9 px-4 text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] bg-slate-100 dark:bg-[#292a2a] rounded-md hover:bg-slate-200 dark:hover:bg-[#3d4948]" @click="showMutationModal = false">Batal</button>
    ...
    <Teleport to="body">
      <div v-if="showCashBoxModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <form class="relative bg-white dark:bg-[#1e2020] rounded-xl shadow-xl w-full max-w-md p-6 space-y-4" @submit.prevent="handleSaveCashBox">
          <h3 class="text-sm font-bold text-slate-900 dark:text-[#e3e2e2]">{{ editingCashBox ? 'Edit Kas' : 'Tambah Kas Baru' }}</h3>
          <div>
            <label class="block text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] mb-1">Nama Kas <span class="text-red-500">*</span></label>
            <input v-model="cashBoxForm.label" type="text" required placeholder="Kas Retail"
              class="w-full h-9 px-3 text-sm border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-md focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" />
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] mb-1">{{ editingCashBox ? 'Saldo (update manual)' : 'Saldo Awal (Rp)' }}</label>
            <input v-model.number="cashBoxForm.balance" type="number" min="0" placeholder="0"
              class="w-full h-9 px-3 text-sm font-mono border border-slate-300 dark:border-[#3d4948] dark:bg-[#1e2020] dark:text-[#e3e2e2] rounded-md focus:border-[#03a29c] focus:ring-1 focus:ring-[#03a29c]/30 outline-none" />
            <p class="text-[10px] text-slate-400 dark:text-[#869392] mt-0.5">{{ editingCashBox ? 'Ubah saldo akan tercatat sebagai mutasi penyesuaian.' : 'Saldo kas saat ini. Bisa 0 kalau baru.' }}</p>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2">
            <button type="button" class="h-9 px-4 text-xs font-semibold text-slate-700 dark:text-[#bcc9c7] bg-slate-100 dark:bg-[#292a2a] rounded-md hover:bg-slate-200 dark:hover:bg-[#3d4948]" @click="showCashBoxModal = false">Batal</button>
            <button type="submit" :disabled="cashBoxSaving" class="h-9 px-4 text-xs font-semibold text-white bg-[#03a29c] rounded-md hover:bg-[#028a85] disabled:opacity-50">{{ editingCashBox ? 'Simpan' : 'Tambah' }}</button>
          </div>
        </form>
      </div>
    </Teleport>
