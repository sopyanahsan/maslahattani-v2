<template>
  <div class="space-y-4">
    ...
    <div v-if="!loading && shifts.length > 0" class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg p-3">
        <p class="text-[11px] text-slate-500 dark:text-[#869392]">Menunggu Review</p>
        <p class="text-lg font-bold text-amber-600 font-mono mt-0.5">
          {{ closedCount }}
        </p>
      </div>
      <div class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg p-3">
        <p class="text-[11px] text-slate-500 dark:text-[#869392]">Sedang Aktif</p>
        <p class="text-lg font-bold text-emerald-600 font-mono mt-0.5">
          {{ openCount }}
        </p>
      </div>
      <div class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg p-3">
        <p class="text-[11px] text-slate-500 dark:text-[#869392]">Total Selisih</p>
        <p
          :class="[
    ...
    <!-- Loading -->
    <div v-if="loading" class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg p-12 text-center">
      <component :is="Loader2Icon" class="w-6 h-6 animate-spin text-blue-600 mx-auto mb-2" />
      <p class="text-xs text-slate-500 dark:text-[#869392]">Memuat shifts…</p>
    </div>

    <!-- Error -->
    <div v-else-if="loadError" class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg p-6">
    ...
    <div
      ...
      class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg p-12 text-center"
    >
      <component :is="ClockIcon" class="w-10 h-10 text-slate-300 mx-auto mb-3" />
      <p class="text-sm font-semibold text-slate-900 dark:text-[#e3e2e2]">Belum ada shift</p>
    ...
    <div v-else class="bg-white dark:bg-[#1e2020] border border-slate-200 dark:border-[#3d4948] rounded-lg shadow-sm overflow-hidden">
      ...
      <table class="hidden md:table w-full">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr
            ...
            :key="shift.id"
            class="hover:bg-slate-50 dark:hover:bg-[#292a2a] transition-colors cursor-pointer"
      ...
      <div class="md:hidden divide-y divide-slate-100">
        <button
          ...
          :key="shift.id"
          type="button"
          class="w-full text-left p-4 hover:bg-slate-50 dark:hover:bg-[#292a2a] transition-colors"
          @click="openDetail(shift.id)"
        >
          ...
          <div v-if="shift.status !== 'OPEN'" class="flex items-center justify-between text-[11px]">
            <span class="text-slate-500 dark:text-[#869392]">Selisih</span>
            <span
              :class="[
                'font-mono font-bold',
                shiftVariance(shift) === 0
