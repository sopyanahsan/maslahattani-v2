<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950">
    ...
    <!-- Tabs -->
    <div class="sticky top-14 z-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6">
      <div class="flex gap-1 py-2 overflow-x-auto">
        <button v-for="t in tabs" :key="t.key" :class="['shrink-0 h-8 px-4 text-xs font-semibold rounded-md transition-colors', activeTab === t.key ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800']" @click="activeTab = t.key">{{ t.label }}</button>
      </div>
    </div>
    ...
    <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <template v-if="activeTab === 'dashboard'">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-center">
            ...
            <p class="text-xl font-bold text-slate-900 dark:text-slate-100 mt-1">{{ formatRupiah(stats.totalRevenue) }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
            <div class="px-5 py-3 border-b border-slate-100 dark:border-slate-800 bg-amber-50 dark:bg-amber-950/20 flex items-center justify-between">
              <h3 class="text-xs font-bold text-amber-800 dark:text-amber-300">Expiring Soon (7 hari)</h3>
              <span class="text-[9px] font-bold text-amber-600 bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 rounded-full">{{ expiringSoon.length }}</span>
            </div>
            <div v-if="expiringSoon.length === 0" class="p-6 text-center">
              <p class="text-xs text-slate-500">Tidak ada yang expire dalam 7 hari.</p>
            </div>
            <div v-else class="divide-y divide-slate-100 dark:divide-slate-800 max-h-[280px] overflow-y-auto">
          ...
          <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
            <div class="px-5 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              <h3 class="text-xs font-bold text-slate-700 dark:text-slate-300">Aktivitas Terbaru</h3>
            </div>
            ...
            <div v-else class="divide-y divide-slate-100 dark:divide-slate-800 max-h-[280px] overflow-y-auto">
              <div v-for="a in recentActivity" :key="a.id" class="px-5 py-3 flex items-start gap-3">
                ...
                <div>
                  <p class="text-xs text-slate-800 dark:text-slate-200"><strong>{{ a.tenantName }}</strong> {{ a.description }}</p>
      ...
      <template v-if="activeTab === 'tenants'">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <input v-model="searchTenant" type="text" placeholder="Cari nama toko / email / WA..." class="flex-1 h-9 px-3 text-sm border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg outline-none focus:border-blue-500" @keyup.enter="fetchTenants" />
          <select v-model="filterStatus" class="h-9 px-3 text-sm border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg" @change="fetchTenants">
            ...
            <table class="w-full">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                <tr v-for="t in tenants" :key="t.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td class="px-4 py-3 text-center text-[10px] text-slate-600 dark:text-slate-400">{{ t.subscription?.endDate ? formatDate(t.subscription.endDate) : '∞' }}</td>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
          ...
          <div v-else class="divide-y divide-slate-100 dark:divide-slate-800">
            <div v-for="p in pendingPayments" :key="p.id" class="px-5 py-4 flex items-center justify-between gap-3">
              <div class="min-w-0">
                <p class="text-xs font-semibold text-slate-900 dark:text-slate-100 truncate">{{ p.tenant?.name }} — {{ p.plan }} ({{ p.cycle }})</p>
              ...
              <div class="flex gap-2 shrink-0">
                ...
                <button class="h-7 px-3 text-[10px] font-bold border border-red-300 text-red-600 rounded-md hover:bg-red-50 dark:hover:bg-red-950/30" @click="rejectPayment(p.id)">Reject</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <template v-if="activeTab === 'reminder'">
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
          <div class="px-5 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <h3 class="text-xs font-bold text-slate-700 dark:text-slate-300">Template WA Reminder — klik untuk copy</h3>
          </div>
          ...
          <div v-else class="divide-y divide-slate-100 dark:divide-slate-800">
            <div v-for="t in expiringSoon" :key="t.id" class="px-5 py-4">
              <div class="flex items-center justify-between mb-2">
                <div>
                  <p class="text-xs font-semibold text-slate-900 dark:text-slate-100">{{ t.name }} — {{ t.ownerName }}</p>
                  <p class="text-[10px] text-slate-500">{{ t.ownerPhone }} • Plan: {{ t.subscription?.plan }} • Expire: {{ formatDate(t.subscription?.endDate) }}</p>
                </div>
                <span :class="['px-1.5 py-0.5 text-[8px] font-bold rounded', daysUntil(t.subscription?.endDate) <= 1 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700']">H-{{ daysUntil(t.subscription?.endDate) }}</span>
              </div>
              <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-3 relative group">
                <pre class="text-[11px] text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed font-sans">{{ generateWaMessage(t) }}</pre>
      ...
      <template v-if="activeTab === 'system'">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
            <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 mb-3">Maintenance Mode</h3>
          ...
          <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
            <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 mb-3">Quick Stats</h3>
            <ul class="space-y-2 text-xs text-slate-600 dark:text-slate-400">
              <li>Tenants total: <strong class="text-slate-900 dark:text-slate-100">{{ stats.totalTenants }}</strong></li>
      ...
      <div v-if="showActivateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
          ...
          <select v-model="activatePlan" class="w-full h-9 px-3 text-sm border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg">
            ...
            <option value="ENTERPRISE">Enterprise — Rp 199rb/bln</option>
          </select>
          ...
          <div class="flex justify-end gap-2">
            <button class="h-9 px-4 text-xs font-semibold border border-slate-300 dark:border-slate-700 rounded-lg" @click="showActivateModal = false">Batal</button>
...
async function extendTenant(t: any) {
  ...
  if (!days) return;
  try {
    await api.put(`/tenants/${t.id}/activate`, { plan: t.subscription?.plan || 'BASIC', cycle: 'MONTHLY' });
    fetchTenants(); fetchStats();
...
function planBadge(plan?: string) {
  ...
  if (plan === 'BASIC') return 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300';
  if (plan === 'TRIAL') return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300';
  return 'bg-slate-100 text-slate-500';
}

function statusBadge(status?: string) {
  if (status === 'ACTIVE' || status === 'LIFETIME') return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300';
  if (status === 'TRIAL') return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300';
  if (status === 'EXPIRED') return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300';
  if (status === 'SUSPENDED') return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300';
  return 'bg-slate-100 text-slate-500';
}
